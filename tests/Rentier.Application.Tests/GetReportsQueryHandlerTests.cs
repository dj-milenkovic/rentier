...
public class GetReportsQueryHandlerTests
{
    ...
    {
        _reportRepo.GetAllAsync(Arg.Any<CancellationToken>())
    ...
    public async Task HandleAsync_ResolvesImporterNameFromDictionary()
    {
        ...
        result.Value[0].ImporterName.Should().Be("My Broker");
    }
    ...
    public async Task HandleAsync_WhenImporterNotFound_UsesUnknownFallback()
    {
        ...
        result.Value[0].ImporterName.Should().Be("Unknown");
    }
    ...
    public async Task HandleAsync_ReturnsCorrectFilingCountPerReport()
    {
        ...
        _reportRepo.GetAllAsync(Arg.Any<CancellationToken>()).Returns([r1, r2]);
        _importerRepo.GetAllAsync(Arg.Any<CancellationToken>()).Returns([importer]);
        ...
        result.Value.First(d => d.Id == r1.Id).FilingCount.Should().Be(5);
        result.Value.First(d => d.Id == r2.Id).FilingCount.Should().Be(2);
    }
